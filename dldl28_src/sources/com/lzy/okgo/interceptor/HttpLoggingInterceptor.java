...
public class HttpLoggingInterceptor implements Interceptor {
    ...
    private void logForRequest(Request request, Connection connection) throws IOException {
        ...
        try {
            try {
                ...
                if (z2) {
                    if (z3) {
                        if (requestBodyBody.get$contentType() != null) {
                            log("\tContent-Type: " + requestBodyBody.get$contentType());
                        }
                    ...
                    if (z && z3) {
                        if (isPlaintext(requestBodyBody.get$contentType())) {
            ...
            try {
                ...
                if (z) {
                    ...
                    if (z2 && okhttp3.internal.http.HttpHeaders.hasBody(responseBuild)) {
                        ...
                        if (isPlaintext(responseBodyBody.get$contentType())) {
                            byte[] byteArray = IOUtils.toByteArray(responseBodyBody.byteStream());
                            log("\tbody:" + new String(byteArray, getCharset(responseBodyBody.get$contentType())));
                            return response.newBuilder().body(ResponseBody.create(responseBodyBody.get$contentType(), byteArray)).build();
                        }
                        log("\tbody: maybe [binary body], omitted!");
                    }
                }
    ...
    private void bodyToString(Request request) {
        try {
            ...
            log("\tbody:" + buffer.readString(getCharset(requestBodyBody.get$contentType())));
        } catch (Exception e) {
            OkLogger.printStackTrace(e);
        }
    }
