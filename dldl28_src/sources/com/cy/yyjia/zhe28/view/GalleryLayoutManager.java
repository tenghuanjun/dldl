...
public class GalleryLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private void fillWithVertical(RecyclerView.Recycler recycler, RecyclerView.State state, int dy) {
        ...
        if (dy >= 0) {
            if (getChildCount() != 0) {
                ...
                decoratedBottom = getDecoratedBottom(childAt3);
                i = position;
            } else {
                i = i4;
                decoratedBottom = -1;
            }
            for (int i5 = i; i5 < getItemCount() && decoratedBottom < endAfterPadding + dy; i5++) {
                Rect rect = getState().mItemsFrames.get(i5);
                View viewForPosition = recycler.getViewForPosition(i5);
                addView(viewForPosition);
                if (rect == null) {
                    rect = new Rect();
                    getState().mItemsFrames.put(i5, rect);
                }
                Rect rect2 = rect;
                measureChildWithMargins(viewForPosition, 0, 0);
                int decoratedMeasuredWidth = getDecoratedMeasuredWidth(viewForPosition);
                ...
                if (decoratedBottom == -1 && i == 0) {
                    ...
                    rect2.set(paddingLeft, decoratedBottom, decoratedMeasuredWidth + paddingLeft, decoratedMeasuredHeight + decoratedBottom);
                }
                layoutDecorated(viewForPosition, rect2.left, rect2.top, rect2.right, rect2.bottom);
                decoratedBottom = rect2.bottom;
        ...
        if (getChildCount() > 0) {
            ...
            decoratedTop = getDecoratedTop(childAt4);
            i4 = position2;
        }
        for (int i6 = i4; i6 >= 0 && decoratedTop > startAfterPadding + dy; i6--) {
            Rect rect3 = getState().mItemsFrames.get(i6);
            View viewForPosition2 = recycler.getViewForPosition(i6);
            addView(viewForPosition2, 0);
            if (rect3 == null) {
                rect3 = new Rect();
                getState().mItemsFrames.put(i6, rect3);
            }
            Rect rect4 = rect3;
            ...
            rect4.set(paddingLeft2, decoratedTop - getDecoratedMeasuredHeight(viewForPosition2), decoratedMeasuredWidth2 + paddingLeft2, decoratedTop);
            layoutDecorated(viewForPosition2, rect4.left, rect4.top, rect4.right, rect4.bottom);
            decoratedTop = rect4.top;
    ...
    private void fillWithHorizontal(RecyclerView.Recycler recycler, RecyclerView.State state, int dx) {
        ...
        if (dx >= 0) {
            ...
            for (int i5 = i; i5 < getItemCount() && decoratedRight < endAfterPadding + dx; i5++) {
                Rect rect = getState().mItemsFrames.get(i5);
                View viewForPosition = recycler.getViewForPosition(i5);
                addView(viewForPosition);
                if (rect == null) {
                ...
                if (decoratedRight == -1 && i == 0) {
                    int paddingLeft = (int) (getPaddingLeft() + ((getHorizontalSpace() - decoratedMeasuredWidth) / 2.0f));
                    rect2.set(paddingLeft, paddingTop, decoratedMeasuredWidth + paddingLeft, decoratedMeasuredHeight + paddingTop);
                } else {
                    rect2.set(decoratedRight, paddingTop, decoratedMeasuredWidth + decoratedRight, decoratedMeasuredHeight + paddingTop);
                }
                layoutDecorated(viewForPosition, rect2.left, rect2.top, rect2.right, rect2.bottom);
                decoratedRight = rect2.right;
                this.mLastVisiblePos = i5;
            }
            return;
        }
        if (getChildCount() > 0) {
            View childAt4 = getChildAt(0);
            int position2 = getPosition(childAt4) - 1;
            decoratedLeft = getDecoratedLeft(childAt4);
            i4 = position2;
        }
        for (int i6 = i4; i6 >= 0 && decoratedLeft > startAfterPadding + dx; i6--) {
            Rect rect3 = getState().mItemsFrames.get(i6);
            View viewForPosition2 = recycler.getViewForPosition(i6);
            addView(viewForPosition2, 0);
            if (rect3 == null) {
            ...
            rect4.set(decoratedLeft - getDecoratedMeasuredWidth(viewForPosition2), paddingTop2, decoratedLeft, getDecoratedMeasuredHeight(viewForPosition2) + paddingTop2);
            layoutDecorated(viewForPosition2, rect4.left, rect4.top, rect4.right, rect4.bottom);
            decoratedLeft = rect4.left;
            this.mFirstVisiblePosition = i6;
        }
    }
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (dx > 0) {
            if (getPosition(getChildAt(getChildCount() - 1)) == getItemCount() - 1) {
                ...
                iMin = Math.max(0, Math.min(dx, (((childAt.getRight() - childAt.getLeft()) / 2) + childAt.getLeft()) - endAfterPadding));
                i = -iMin;
            }
        } else if (this.mFirstVisiblePosition == 0) {
            View childAt2 = getChildAt(0);
            iMin = Math.min(0, Math.max(dx, (((childAt2.getRight() - childAt2.getLeft()) / 2) + childAt2.getLeft()) - endAfterPadding));
            i = -iMin;
        }
        int i2 = -i;
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (dy > 0) {
            if (getPosition(getChildAt(getChildCount() - 1)) == getItemCount() - 1) {
                View childAt = getChildAt(getChildCount() - 1);
                iMin = Math.max(0, Math.min(dy, (((getDecoratedBottom(childAt) - getDecoratedTop(childAt)) / 2) + getDecoratedTop(childAt)) - endAfterPadding));
                i = -iMin;
            }
        } else if (this.mFirstVisiblePosition == 0) {
            View childAt2 = getChildAt(0);
            iMin = Math.min(0, Math.max(dy, (((getDecoratedBottom(childAt2) - getDecoratedTop(childAt2)) / 2) + getDecoratedTop(childAt2)) - endAfterPadding));
            i = -iMin;
        }
        int i2 = -i;
        getState().mScrollDelta = i2;
    ...
    private class InnerScrollListener extends RecyclerView.OnScrollListener {
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            ...
            View viewFindSnapView = GalleryLayoutManager.this.mSnapHelper.findSnapView(recyclerView.getLayoutManager());
            if (viewFindSnapView == null || (position = recyclerView.getLayoutManager().getPosition(viewFindSnapView)) == GalleryLayoutManager.this.mCurSelectedPosition) {
                return;
            }
            if (GalleryLayoutManager.this.mCurSelectedView != null) {
                GalleryLayoutManager.this.mCurSelectedView.setSelected(false);
            }
            GalleryLayoutManager.this.mCurSelectedView = viewFindSnapView;
            ...
            if (!GalleryLayoutManager.this.mCallbackInFling && this.mState != 0) {
                ...
                GalleryLayoutManager.this.mOnItemSelectedListener.onItemSelected(recyclerView, viewFindSnapView, GalleryLayoutManager.this.mCurSelectedPosition);
            }
        }
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            if (newState == 0) {
                ...
                if (viewFindSnapView != null) {
                    int position = recyclerView.getLayoutManager().getPosition(viewFindSnapView);
    ...
    private class GallerySmoothScroller extends LinearSmoothScroller {
        ...
        protected void onTargetFound(View targetView, RecyclerView.State state, RecyclerView.SmoothScroller.Action action) {
            int iCalculateDxToMakeCentral = calculateDxToMakeCentral(targetView);
            int iCalculateDyToMakeCentral = calculateDyToMakeCentral(targetView);
            int iCalculateTimeForDeceleration = calculateTimeForDeceleration((int) Math.sqrt((iCalculateDxToMakeCentral * iCalculateDxToMakeCentral) + (iCalculateDyToMakeCentral * iCalculateDyToMakeCentral)));
            if (iCalculateTimeForDeceleration > 0) {
                action.update(-iCalculateDxToMakeCentral, -iCalculateDyToMakeCentral, iCalculateTimeForDeceleration, this.mDecelerateInterpolator);
            }
        }
    }
}
