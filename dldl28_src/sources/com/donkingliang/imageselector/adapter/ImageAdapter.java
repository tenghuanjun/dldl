...
public class ImageAdapter extends RecyclerView.Adapter<ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder viewHolder, int i) {
        if (getItemViewType(i) == 2) {
            ...
            Glide.with(this.mContext).load(this.isAndroidQ ? image.getUri() : image.getPath()).apply((BaseRequestOptions<?>) new RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE)).into(viewHolder.ivImage);
