...
final class SplineBasedFloatDecayAnimationSpec implements FloatDecayAnimationSpec {
    ...
    public float getValueFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        ...
        return initialValue + (flingDistance(initialVelocity) * AndroidFlingSpline.FlingResult.m499getDistanceCoefficientimpl(AndroidFlingSpline.INSTANCE.m494flingPositionLfoxSSI(durationNanos > 0 ? playTimeNanos / durationNanos : 1.0f)));
    }
    ...
    public float getVelocityFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        long durationNanos = getDurationNanos(0.0f, initialVelocity);
        return ((AndroidFlingSpline.FlingResult.m500getVelocityCoefficientimpl(AndroidFlingSpline.INSTANCE.m494flingPositionLfoxSSI(durationNanos > 0 ? playTimeNanos / durationNanos : 1.0f)) * flingDistance(initialVelocity)) / durationNanos) * 1.0E9f;
    }
}
