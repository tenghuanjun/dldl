...
public abstract class LazyGridMeasuredLineProvider {
    ...
    public final long m741childConstraintsJhjzzOo$foundation_release(int startSlot, int span) {
        int i;
        if (span == 1) {
        ...
        if (this.isVertical) {
            return Constraints.INSTANCE.m5838fixedWidthOenEA2s(iCoerceAtLeast);
        }
        return Constraints.INSTANCE.m5837fixedHeightOenEA2s(iCoerceAtLeast);
    }

    /* JADX INFO: renamed from: itemConstraints-OenEA2s, reason: not valid java name */
    public final long m742itemConstraintsOenEA2s(int itemIndex) {
        LazyGridSpanLayoutProvider lazyGridSpanLayoutProvider = this.spanLayoutProvider;
        return m741childConstraintsJhjzzOo$foundation_release(0, lazyGridSpanLayoutProvider.spanOf(itemIndex, lazyGridSpanLayoutProvider.getSlotsPerLine()));
    }
    ...
    public final LazyGridMeasuredLine getAndMeasure(int lineIndex) {
        ...
        for (int i3 = 0; i3 < size; i3++) {
            int iM719getCurrentLineSpanimpl = GridItemSpan.m719getCurrentLineSpanimpl(lineConfiguration.getSpans().get(i3).getPackedValue());
            LazyGridMeasuredItem lazyGridMeasuredItemM740getAndMeasure3p2s80s = this.measuredItemProvider.m740getAndMeasure3p2s80s(lineConfiguration.getFirstItemIndex() + i3, i, m741childConstraintsJhjzzOo$foundation_release(i2, iM719getCurrentLineSpanimpl));
            i2 += iM719getCurrentLineSpanimpl;
            Unit unit = Unit.INSTANCE;
            lazyGridMeasuredItemArr[i3] = lazyGridMeasuredItemM740getAndMeasure3p2s80s;
        }
