...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m3548toArgb8_81llA(colors.get(i).m3504unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                fM3220getYimpl = Offset.m3219getXimpl(packedValue);
            } else {
                fM3220getYimpl = Offset.m3220getYimpl(packedValue);
            }
            fArr[i] = fM3220getYimpl;
        }
        return fArr;
    }
}
