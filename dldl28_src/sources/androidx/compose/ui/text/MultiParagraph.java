...
public final class MultiParagraph {
    ...
    private MultiParagraph(MultiParagraphIntrinsics multiParagraphIntrinsics, long j, int i, boolean z) {
        ...
        this.maxLines = i;
        if (Constraints.m5830getMinWidthimpl(j) != 0 || Constraints.m5829getMinHeightimpl(j) != 0) {
        ...
        while (i3 < size) {
            ...
            if (Constraints.m5823getHasBoundedHeightimpl(j)) {
                iM5827getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m5827getMaxHeightimpl(j) - ParagraphKt.ceilToInt(f), i2);
            } else {
                iM5827getMaxHeightimpl = Constraints.m5827getMaxHeightimpl(j);
            }
            Paragraph paragraphM5261Paragraph_EkL_Y = ParagraphKt.m5261Paragraph_EkL_Y(intrinsics, ConstraintsKt.Constraints$default(0, iM5828getMaxWidthimpl, 0, iM5827getMaxHeightimpl, 5, null), this.maxLines - i4, z);
            float height = f + paragraphM5261Paragraph_EkL_Y.getHeight();
            int lineCount = i4 + paragraphM5261Paragraph_EkL_Y.getLineCount();
            List<ParagraphIntrinsicInfo> list = infoList$ui_text_release;
            arrayList.add(new ParagraphInfo(paragraphM5261Paragraph_EkL_Y, paragraphIntrinsicInfo.getStartIndex(), paragraphIntrinsicInfo.getEndIndex(), i4, lineCount, f, height));
            if (paragraphM5261Paragraph_EkL_Y.getDidExceedMaxLines() || (lineCount == this.maxLines && i3 != CollectionsKt.getLastIndex(this.intrinsics.getInfoList$ui_text_release()))) {
                z2 = true;
                i4 = lineCount;
                f = height;
                break;
        ...
        this.width = Constraints.m5828getMaxWidthimpl(j);
    ...
    public static /* synthetic */ void m5242paintRPmYEkk$default(MultiParagraph multiParagraph, Canvas canvas, long j, Shadow shadow, TextDecoration textDecoration, int i, Object obj) {
        if ((i & 2) != 0) {
            j = Color.INSTANCE.m3530getUnspecified0d7_KjU();
        }
        multiParagraph.m5248paintRPmYEkk(canvas, j, (i & 4) != 0 ? null : shadow, (i & 8) != 0 ? null : textDecoration);
    }
    ...
    public final /* synthetic */ void m5248paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo5219paintRPmYEkk(canvas, color, shadow, decoration);
    ...
    public final void m5247paintLG529CI(Canvas canvas, long color, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo5218paintLG529CI(canvas, color, shadow, decoration, drawStyle, blendMode);
            canvas.translate(0.0f, paragraphInfo.getParagraph().getHeight());
        }
        canvas.restore();
    }
    ...
    public final void m5249painthn5TExg(Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        AndroidMultiParagraphDraw_androidKt.m5616drawMultiParagraph7AXcY_I(this, canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
    }
    ...
    public final Path getPathForRange(final int start, final int end) {
        if (start >= 0 && start <= end && end <= getAnnotatedString().getText().length()) {
            ...
            MultiParagraphKt.m5250findParagraphsByRangeSbBc2M(this.paragraphInfoList, TextRangeKt.TextRange(start, end), new Function1<ParagraphInfo, Unit>() { // from class: androidx.compose.ui.text.MultiParagraph.getPathForRange.2
                ...
                public final void invoke2(ParagraphInfo paragraphInfo) {
                    Path.CC.m3773addPathUv8p0NA$default(Path, paragraphInfo.toGlobal(paragraphInfo.getParagraph().getPathForRange(paragraphInfo.toLocalIndex(start), paragraphInfo.toLocalIndex(end))), 0L, 2, null);
                }
            });
            return Path;
        }
    ...
    public final int m5245getOffsetForPositionk4lQ0M(long position) {
        int lastIndex;
        if (Offset.m3220getYimpl(position) <= 0.0f) {
            lastIndex = 0;
        } else {
            lastIndex = Offset.m3220getYimpl(position) >= this.height ? CollectionsKt.getLastIndex(this.paragraphInfoList) : MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.m3220getYimpl(position));
        }
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(lastIndex);
        if (paragraphInfo.getLength() == 0) {
            return paragraphInfo.getStartIndex();
        }
        return paragraphInfo.toGlobalIndex(paragraphInfo.getParagraph().mo5216getOffsetForPositionk4lQ0M(paragraphInfo.m5258toLocalMKHz9U(position)));
    }
    ...
    public final float[] m5244fillBoundingBoxes8ffj60Q(final long range, final float[] array, int arrayStart) {
        requireIndexInRange(TextRange.m5366getMinimpl(range));
        requireIndexInRangeInclusiveEnd(TextRange.m5365getMaximpl(range));
        final Ref.IntRef intRef = new Ref.IntRef();
        intRef.element = arrayStart;
        final Ref.FloatRef floatRef = new Ref.FloatRef();
        MultiParagraphKt.m5250findParagraphsByRangeSbBc2M(this.paragraphInfoList, range, new Function1<ParagraphInfo, Unit>() { // from class: androidx.compose.ui.text.MultiParagraph$fillBoundingBoxes$1
            /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
            {
                super(1);
            }
            ...
            public final void invoke2(ParagraphInfo paragraphInfo) {
                ...
                long jTextRange = TextRangeKt.TextRange(paragraphInfo.toLocalIndex(paragraphInfo.getStartIndex() > TextRange.m5366getMinimpl(j) ? paragraphInfo.getStartIndex() : TextRange.m5366getMinimpl(j)), paragraphInfo.toLocalIndex(paragraphInfo.getEndIndex() < TextRange.m5365getMaximpl(j) ? paragraphInfo.getEndIndex() : TextRange.m5365getMaximpl(j)));
                paragraphInfo.getParagraph().mo5214fillBoundingBoxes8ffj60Q(jTextRange, fArr, intRef2.element);
                int iM5364getLengthimpl = intRef2.element + (TextRange.m5364getLengthimpl(jTextRange) * 4);
                for (int i = intRef2.element; i < iM5364getLengthimpl; i += 4) {
                    int i2 = i + 1;
                    fArr[i2] = fArr[i2] + floatRef2.element;
                    int i3 = i + 3;
                    fArr[i3] = fArr[i3] + floatRef2.element;
                }
                intRef2.element = iM5364getLengthimpl;
    ...
    public final long m5246getWordBoundaryjx7JFs(int offset) {
        int iFindParagraphByIndex;
        requireIndexInRangeInclusiveEnd(offset);
        if (offset == getAnnotatedString().length()) {
            iFindParagraphByIndex = CollectionsKt.getLastIndex(this.paragraphInfoList);
        ...
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(iFindParagraphByIndex);
        return paragraphInfo.m5257toGlobalGEjPoXI(paragraphInfo.getParagraph().mo5217getWordBoundaryjx7JFs(paragraphInfo.toLocalIndex(offset)));
    }
