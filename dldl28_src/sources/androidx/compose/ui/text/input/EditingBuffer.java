...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m5366getMinimpl(j);
        this.selectionEnd = TextRange.m5365getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int iM5366getMinimpl = TextRange.m5366getMinimpl(j);
        int iM5365getMaximpl = TextRange.m5365getMaximpl(j);
        if (iM5366getMinimpl < 0 || iM5366getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + iM5366getMinimpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM5365getMaximpl < 0 || iM5365getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + iM5365getMaximpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM5366getMinimpl <= iM5365getMaximpl) {
            return;
        }
        throw new IllegalArgumentException("Do not set reversed range: " + iM5366getMinimpl + " > " + iM5365getMaximpl);
    }
    ...
    public final TextRange m5529getCompositionMzsxiRA$ui_text_release() {
    ...
    public final long m5530getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int start, int end) {
        ...
        long jM5531updateRangeAfterDeletepWDy79M = EditingBufferKt.m5531updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), jTextRange);
        setSelectionStart(TextRange.m5366getMinimpl(jM5531updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m5365getMaximpl(jM5531updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long jM5531updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m5531updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), jTextRange);
            if (TextRange.m5362getCollapsedimpl(jM5531updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
            } else {
                this.compositionStart = TextRange.m5366getMinimpl(jM5531updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m5365getMaximpl(jM5531updateRangeAfterDeletepWDy79M2);
            }
        }
    }
