...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f, State<Color> state, State<RippleAlpha> state2, ViewGroup viewGroup) {
        ...
        this.rippleSize = Size.INSTANCE.m3297getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        int iRoundToInt;
        this.rippleSize = contentDrawScope.mo3973getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            iRoundToInt = MathKt.roundToInt(RippleAnimationKt.m1252getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo3973getSizeNHjbRc()));
        } else {
            iRoundToInt = contentDrawScope.mo339roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = iRoundToInt;
        long jM3504unboximpl = this.color.getValue().m3504unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m1257drawStateLayerH2RKhps(contentDrawScope2, this.radius, jM3504unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m1256updateRipplePropertiesbiQXAtU(contentDrawScope.mo3973getSizeNHjbRc(), this.rippleRadius, jM3504unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m1255addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m3504unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
