...
public final class TrieNode<K, V> {
    ...
    private final TrieNode<K, V> mutableCollisionPutAll(TrieNode<K, V> otherNode, DeltaCounter intersectionCounter, MutabilityOwnership owner) {
        CommonFunctionsKt.m3076assert(this.nodeMap == 0);
        CommonFunctionsKt.m3076assert(this.dataMap == 0);
        CommonFunctionsKt.m3076assert(otherNode.nodeMap == 0);
        CommonFunctionsKt.m3076assert(otherNode.dataMap == 0);
        Object[] objArr = this.buffer;
    ...
    public final TrieNode<K, V> mutablePutAll(TrieNode<K, V> otherNode, int shift, DeltaCounter intersectionCounter, PersistentHashMapBuilder<K, V> mutator) {
        ...
        while (i8 != 0) {
            ...
            trieNode.buffer[(r5.length - 1) - i9] = mutablePutAllFromOtherNodeCell(otherNode, iLowestOneBit2, shift, intersectionCounter, mutator);
            i9++;
            i8 ^= iLowestOneBit2;
        }
        while (i6 != 0) {
