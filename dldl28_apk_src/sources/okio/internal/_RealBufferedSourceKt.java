...
public final class _RealBufferedSourceKt {
    public static final long commonRead(RealBufferedSource realBufferedSource, Buffer sink, long j) {
        ...
        if (realBufferedSource.bufferField.size() == 0 && realBufferedSource.source.read(realBufferedSource.bufferField, IjkMediaMeta.AV_CH_TOP_FRONT_CENTER) == -1) {
            return -1L;
        }
    ...
    public static final boolean commonExhausted(RealBufferedSource realBufferedSource) {
        ...
        return realBufferedSource.bufferField.exhausted() && realBufferedSource.source.read(realBufferedSource.bufferField, IjkMediaMeta.AV_CH_TOP_FRONT_CENTER) == -1;
    }
    ...
    public static final boolean commonRequest(RealBufferedSource realBufferedSource, long j) {
        ...
        while (realBufferedSource.bufferField.size() < j) {
            if (realBufferedSource.source.read(realBufferedSource.bufferField, IjkMediaMeta.AV_CH_TOP_FRONT_CENTER) == -1) {
                return false;
            }
        }
        return true;
    ...
    public static final int commonSelect(RealBufferedSource realBufferedSource, Options options) throws EOFException {
        ...
        } while (realBufferedSource.source.read(realBufferedSource.bufferField, IjkMediaMeta.AV_CH_TOP_FRONT_CENTER) != -1);
        return -1;
    }
    ...
    public static final long commonIndexOf(RealBufferedSource realBufferedSource, byte b, long j, long j2) {
        ...
        while (j < j2) {
            ...
            if (size >= j2 || realBufferedSource.source.read(realBufferedSource.bufferField, IjkMediaMeta.AV_CH_TOP_FRONT_CENTER) == -1) {
                break;
            }
            j = Math.max(j, size);
        }
