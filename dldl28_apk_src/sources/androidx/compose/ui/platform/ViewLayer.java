...
public final class ViewLayer extends View implements OwnedLayer, GraphicLayerInfo {
    ...
    public ViewLayer(AndroidComposeView androidComposeView, DrawChildContainer drawChildContainer, Function1<? super Canvas, Unit> function1, Function0<Unit> function0) {
        ...
        this.mTransformOrigin = TransformOrigin.INSTANCE.m3893getCenterSzJe1aQ();
    ...
    public void updateLayerProperties(ReusableGraphicsLayerScope scope, LayoutDirection layoutDirection, Density density) {
        ...
        if ((mutatedFields$ui_release & 4096) != 0) {
            long jMo3665getTransformOriginSzJe1aQ = scope.getTransformOrigin();
            this.mTransformOrigin = jMo3665getTransformOriginSzJe1aQ;
            setPivotX(TransformOrigin.m3888getPivotFractionXimpl(jMo3665getTransformOriginSzJe1aQ) * getWidth());
            setPivotY(TransformOrigin.m3889getPivotFractionYimpl(this.mTransformOrigin) * getHeight());
        }
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            if ((mutatedFields$ui_release & 64) != 0) {
                ViewLayerVerificationHelper28.INSTANCE.setOutlineAmbientShadowColor(this, ColorKt.m3548toArgb8_81llA(scope.getAmbientShadowColor()));
            }
            if ((mutatedFields$ui_release & 128) != 0) {
                ViewLayerVerificationHelper28.INSTANCE.setOutlineSpotShadowColor(this, ColorKt.m3548toArgb8_81llA(scope.getSpotShadowColor()));
            }
        }
        ...
        if ((mutatedFields$ui_release & 32768) != 0) {
            int iMo3662getCompositingStrategyNrFUSI = scope.getCompositingStrategy();
            if (CompositingStrategy.m3580equalsimpl0(iMo3662getCompositingStrategyNrFUSI, CompositingStrategy.INSTANCE.m3586getOffscreenNrFUSI())) {
                setLayerType(2, null);
            } else if (CompositingStrategy.m3580equalsimpl0(iMo3662getCompositingStrategyNrFUSI, CompositingStrategy.INSTANCE.m3585getModulateAlphaNrFUSI())) {
                setLayerType(0, null);
    ...
    public boolean mo5041isInLayerk4lQ0M(long position) {
        float fM3219getXimpl = Offset.m3219getXimpl(position);
        float fM3220getYimpl = Offset.m3220getYimpl(position);
        if (this.clipToBounds) {
            return 0.0f <= fM3219getXimpl && fM3219getXimpl < ((float) getWidth()) && 0.0f <= fM3220getYimpl && fM3220getYimpl < ((float) getHeight());
        }
        if (getClipToOutline()) {
            return this.outlineResolver.m5142isInOutlinek4lQ0M(position);
        }
        return true;
    }
    ...
    public void mo5044resizeozmzZPI(long size) {
        int iM6056getWidthimpl = IntSize.m6056getWidthimpl(size);
        int iM6055getHeightimpl = IntSize.m6055getHeightimpl(size);
        if (iM6056getWidthimpl == getWidth() && iM6055getHeightimpl == getHeight()) {
            return;
        }
        float f = iM6056getWidthimpl;
        setPivotX(TransformOrigin.m3888getPivotFractionXimpl(this.mTransformOrigin) * f);
        float f2 = iM6055getHeightimpl;
        setPivotY(TransformOrigin.m3889getPivotFractionYimpl(this.mTransformOrigin) * f2);
        this.outlineResolver.m5143updateuvyYCjk(SizeKt.Size(f, f2));
        updateOutlineResolver();
        layout(getLeft(), getTop(), getLeft() + iM6056getWidthimpl, getTop() + iM6055getHeightimpl);
        resetClipBounds();
        this.matrixCache.invalidate();
    }
    ...
    public void mo5043movegyyYBs(long position) {
        int iM6014getXimpl = IntOffset.m6014getXimpl(position);
        if (iM6014getXimpl != getLeft()) {
            offsetLeftAndRight(iM6014getXimpl - getLeft());
            this.matrixCache.invalidate();
        }
        int iM6015getYimpl = IntOffset.m6015getYimpl(position);
        if (iM6015getYimpl != getTop()) {
            offsetTopAndBottom(iM6015getYimpl - getTop());
            this.matrixCache.invalidate();
        }
    }
    ...
    public long mo5042mapOffset8S9VItk(long point, boolean inverse) {
        if (inverse) {
            float[] fArrM5119calculateInverseMatrixbWbORWo = this.matrixCache.m5119calculateInverseMatrixbWbORWo(this);
            return fArrM5119calculateInverseMatrixbWbORWo != null ? androidx.compose.ui.graphics.Matrix.m3739mapMKHz9U(fArrM5119calculateInverseMatrixbWbORWo, point) : Offset.INSTANCE.m3233getInfiniteF1C5BW0();
        }
        return androidx.compose.ui.graphics.Matrix.m3739mapMKHz9U(this.matrixCache.m5120calculateMatrixGrdbGEg(this), point);
    }
    ...
    public void mapBounds(MutableRect rect, boolean inverse) {
        if (inverse) {
            float[] fArrM5119calculateInverseMatrixbWbORWo = this.matrixCache.m5119calculateInverseMatrixbWbORWo(this);
            if (fArrM5119calculateInverseMatrixbWbORWo != null) {
                androidx.compose.ui.graphics.Matrix.m3741mapimpl(fArrM5119calculateInverseMatrixbWbORWo, rect);
                return;
            } else {
        ...
        androidx.compose.ui.graphics.Matrix.m3741mapimpl(this.matrixCache.m5120calculateMatrixGrdbGEg(this), rect);
    }
    ...
    public void mo5045transform58bKbWc(float[] matrix) {
        androidx.compose.ui.graphics.Matrix.m3750timesAssign58bKbWc(matrix, this.matrixCache.m5120calculateMatrixGrdbGEg(this));
    }

    @Override // androidx.compose.ui.node.OwnedLayer
    /* JADX INFO: renamed from: inverseTransform-58bKbWc */
    public void mo5040inverseTransform58bKbWc(float[] matrix) {
        float[] fArrM5119calculateInverseMatrixbWbORWo = this.matrixCache.m5119calculateInverseMatrixbWbORWo(this);
        if (fArrM5119calculateInverseMatrixbWbORWo != null) {
            androidx.compose.ui.graphics.Matrix.m3750timesAssign58bKbWc(matrix, fArrM5119calculateInverseMatrixbWbORWo);
        }
    }
