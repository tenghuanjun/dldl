...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m5139toOffsetUv8p0NA(int[] iArr, long j) {
        float fCoerceAtLeast;
        float fCoerceAtLeast2;
        if (Offset.m3219getXimpl(j) >= 0.0f) {
            fCoerceAtLeast = RangesKt.coerceAtMost(reverseAxis(iArr[0]), Offset.m3219getXimpl(j));
        } else {
            fCoerceAtLeast = RangesKt.coerceAtLeast(reverseAxis(iArr[0]), Offset.m3219getXimpl(j));
        }
        if (Offset.m3220getYimpl(j) >= 0.0f) {
            fCoerceAtLeast2 = RangesKt.coerceAtMost(reverseAxis(iArr[1]), Offset.m3220getYimpl(j));
        } else {
            fCoerceAtLeast2 = RangesKt.coerceAtLeast(reverseAxis(iArr[1]), Offset.m3220getYimpl(j));
        }
    ...
    public static final int m5140toViewTypeGyEprt8(int i) {
        return !NestedScrollSource.m4544equalsimpl0(i, NestedScrollSource.INSTANCE.m4549getDragWNlRxjI()) ? 1 : 0;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* JADX INFO: renamed from: getScrollAxes-k-4lQ0M, reason: not valid java name */
    public static final int m5138getScrollAxesk4lQ0M(long j) {
        int i = Math.abs(Offset.m3219getXimpl(j)) >= 0.5f ? 1 : 0;
        return Math.abs(Offset.m3220getYimpl(j)) >= 0.5f ? i | 2 : i;
    }
