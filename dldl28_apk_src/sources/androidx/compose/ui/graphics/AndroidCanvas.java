...
public final class AndroidCanvas implements Canvas {
    ...
    public /* synthetic */ void mo3315clipRectmtrdDE(androidx.compose.ui.geometry.Rect rect, int i) {
        Canvas.CC.m3464$default$clipRectmtrdDE(this, rect, i);
    }
    ...
    public void mo3316concat58bKbWc(float[] matrix) {
        if (MatrixKt.m3757isIdentity58bKbWc(matrix)) {
            return;
        }
        android.graphics.Matrix matrix2 = new android.graphics.Matrix();
        AndroidMatrixConversions_androidKt.m3331setFromEL8BTi8(matrix2, matrix);
        this.internalCanvas.concat(matrix2);
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* JADX INFO: renamed from: clipRect-N_I0leg, reason: not valid java name */
    public void mo3314clipRectN_I0leg(float left, float top, float right, float bottom, int clipOp) {
        this.internalCanvas.clipRect(left, top, right, bottom, m3324toRegionOp7u2Bmg(clipOp));
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* JADX INFO: renamed from: clipPath-mtrdD-E, reason: not valid java name */
    public void mo3313clipPathmtrdDE(Path path, int clipOp) {
        android.graphics.Canvas canvas = this.internalCanvas;
        if (path instanceof AndroidPath) {
            canvas.clipPath(((AndroidPath) path).getInternalPath(), m3324toRegionOp7u2Bmg(clipOp));
            return;
        }
    ...
    public final Region.Op m3324toRegionOp7u2Bmg(int i) {
        return ClipOp.m3478equalsimpl0(i, ClipOp.INSTANCE.m3482getDifferencertfAjoo()) ? Region.Op.DIFFERENCE : Region.Op.INTERSECT;
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* JADX INFO: renamed from: drawLine-Wko1d7g, reason: not valid java name */
    public void mo3320drawLineWko1d7g(long p1, long p2, Paint paint) {
        this.internalCanvas.drawLine(Offset.m3219getXimpl(p1), Offset.m3220getYimpl(p1), Offset.m3219getXimpl(p2), Offset.m3220getYimpl(p2), paint.getInternalPaint());
    }
    ...
    public void mo3317drawCircle9KIMszo(long center, float radius, Paint paint) {
        this.internalCanvas.drawCircle(Offset.m3219getXimpl(center), Offset.m3220getYimpl(center), radius, paint.getInternalPaint());
    }
    ...
    public void mo3318drawImaged4ec7I(ImageBitmap image, long topLeftOffset, Paint paint) {
        this.internalCanvas.drawBitmap(AndroidImageBitmap_androidKt.asAndroidBitmap(image), Offset.m3219getXimpl(topLeftOffset), Offset.m3220getYimpl(topLeftOffset), paint.getInternalPaint());
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* JADX INFO: renamed from: drawImageRect-HPBpro0, reason: not valid java name */
    public void mo3319drawImageRectHPBpro0(ImageBitmap image, long srcOffset, long srcSize, long dstOffset, long dstSize, Paint paint) {
        ...
        rect.left = IntOffset.m6014getXimpl(srcOffset);
        rect.top = IntOffset.m6015getYimpl(srcOffset);
        rect.right = IntOffset.m6014getXimpl(srcOffset) + IntSize.m6056getWidthimpl(srcSize);
        rect.bottom = IntOffset.m6015getYimpl(srcOffset) + IntSize.m6055getHeightimpl(srcSize);
        Unit unit = Unit.INSTANCE;
        Rect rect2 = this.dstRect;
        Intrinsics.checkNotNull(rect2);
        rect2.left = IntOffset.m6014getXimpl(dstOffset);
        rect2.top = IntOffset.m6015getYimpl(dstOffset);
        rect2.right = IntOffset.m6014getXimpl(dstOffset) + IntSize.m6056getWidthimpl(dstSize);
        rect2.bottom = IntOffset.m6015getYimpl(dstOffset) + IntSize.m6055getHeightimpl(dstSize);
    ...
    public void mo3321drawPointsO7TthRY(int pointMode, List<Offset> points, Paint paint) {
        if (PointMode.m3803equalsimpl0(pointMode, PointMode.INSTANCE.m3807getLinesr_lszbg())) {
            drawLines(points, paint, 2);
        } else if (PointMode.m3803equalsimpl0(pointMode, PointMode.INSTANCE.m3809getPolygonr_lszbg())) {
            drawLines(points, paint, 1);
        } else if (PointMode.m3803equalsimpl0(pointMode, PointMode.INSTANCE.m3808getPointsr_lszbg())) {
    ...
    private final void drawLines(List<Offset> points, Paint paint, int stepBy) {
        if (points.size() >= 2) {
            ...
            while (i < points.size() - 1) {
                ...
                this.internalCanvas.drawLine(Offset.m3219getXimpl(packedValue), Offset.m3220getYimpl(packedValue), Offset.m3219getXimpl(packedValue2), Offset.m3220getYimpl(packedValue2), internalPaint);
                i += stepBy;
            }
        }
    }
    ...
    public void mo3322drawRawPointsO7TthRY(int pointMode, float[] points, Paint paint) {
    ...
    public void mo3323drawVerticesTPEHhCM(Vertices vertices, int blendMode, Paint paint) {
        this.internalCanvas.drawVertices(AndroidVertexMode_androidKt.m3367toAndroidVertexModeJOOmi9M(vertices.getVertexMode()), vertices.getPositions().length, vertices.getPositions(), 0, vertices.getTextureCoordinates(), 0, vertices.getColors(), 0, vertices.getIndices(), 0, vertices.getIndices().length, paint.getInternalPaint());
    }
    ...
    private final void drawPoints(List<Offset> points, Paint paint) {
        ...
        for (int i = 0; i < size; i++) {
            long packedValue = points.get(i).getPackedValue();
            this.internalCanvas.drawPoint(Offset.m3219getXimpl(packedValue), Offset.m3220getYimpl(packedValue), paint.getInternalPaint());
        }
    }
}
