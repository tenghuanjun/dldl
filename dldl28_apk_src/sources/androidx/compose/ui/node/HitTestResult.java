...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long jM4909findBestHitDistanceptXAw2c = m4909findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m4904getDistanceimpl(jM4909findBestHitDistanceptXAw2c) < 0.0f && DistanceAndInLayer.m4906isInLayerimpl(jM4909findBestHitDistanceptXAw2c);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        return DistanceAndInLayer.m4900compareToS_HNhKs(m4909findBestHitDistanceptXAw2c(), HitTestResultKt.DistanceAndInLayer(distanceFromEdge, isInLayer)) > 0;
    }

    /* JADX INFO: renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m4909findBestHitDistanceptXAw2c() {
        long jDistanceAndInLayer = HitTestResultKt.DistanceAndInLayer(Float.POSITIVE_INFINITY, false);
        int i = this.hitDepth + 1;
        int lastIndex = CollectionsKt.getLastIndex(this);
        if (i <= lastIndex) {
            while (true) {
                long jM4901constructorimpl = DistanceAndInLayer.m4901constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                if (DistanceAndInLayer.m4900compareToS_HNhKs(jM4901constructorimpl, jDistanceAndInLayer) < 0) {
                    jDistanceAndInLayer = jM4901constructorimpl;
                }
                if (DistanceAndInLayer.m4904getDistanceimpl(jDistanceAndInLayer) < 0.0f && DistanceAndInLayer.m4906isInLayerimpl(jDistanceAndInLayer)) {
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(hitTestResult) && DistanceAndInLayer.m4900compareToS_HNhKs(jM4909findBestHitDistanceptXAw2c, m4909findBestHitDistanceptXAw2c()) > 0) {
