...
public final /* data */ class BiasAbsoluteAlignment implements Alignment {
    ...
    public long mo3096alignKFBX0sM(long size, long space, LayoutDirection layoutDirection) {
        long jIntSize = IntSizeKt.IntSize(IntSize.m6056getWidthimpl(space) - IntSize.m6056getWidthimpl(size), IntSize.m6055getHeightimpl(space) - IntSize.m6055getHeightimpl(size));
        float f = 1;
        return IntOffsetKt.IntOffset(MathKt.roundToInt((IntSize.m6056getWidthimpl(jIntSize) / 2.0f) * (this.horizontalBias + f)), MathKt.roundToInt((IntSize.m6055getHeightimpl(jIntSize) / 2.0f) * (f + this.verticalBias)));
    }
