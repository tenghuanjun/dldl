...
public class ImageModel {
    ...
    public static synchronized ArrayList<Image> loadImage(Context context) {
        ...
        Cursor cursorQuery = context.getContentResolver().query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, new String[]{"_data", "_display_name", "date_added", DBHelper.COL_ID, "mime_type", "_size"}, "_size>0", null, "date_added DESC");
        arrayList = new ArrayList<>();
        if (cursorQuery != null) {
            while (cursorQuery.moveToNext()) {
                long j = cursorQuery.getLong(cursorQuery.getColumnIndex(DBHelper.COL_ID));
