...
public class DataBindingHelper {
    ...
    public static void setImg(ImageView imageView, String url, Drawable placeholder) {
        RequestOptions requestOptionsOverride = new RequestOptions().override(Integer.MIN_VALUE, Integer.MIN_VALUE);
        if (placeholder != null) {
            requestOptionsOverride = requestOptionsOverride.error(placeholder).placeholder(placeholder);
        }
        Glide.with(imageView.getContext()).load(url).apply((BaseRequestOptions<?>) requestOptionsOverride).into(imageView);
    }
    ...
    public static void setGameIcon(ImageView imageView, String gameIcon) {
        Glide.with(imageView).load(gameIcon).apply((BaseRequestOptions<?>) new RequestOptions().placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher)).override(imageView.getWidth(), imageView.getHeight()).into(imageView);
    }

    public static void setUserIcon(ImageView imageView, String userIcon) {
        Glide.with(imageView).load(userIcon).apply((BaseRequestOptions<?>) new RequestOptions().circleCrop().placeholder(R.mipmap.icon_user_default).error(R.mipmap.icon_user_default)).into(imageView);
    }

    public static void setDealIcon(ImageView imageView, String dealIcon) {
        Glide.with(imageView).load(dealIcon).apply((BaseRequestOptions<?>) new RequestOptions().centerCrop().placeholder(R.mipmap.ic_deal_placeholder).error(R.mipmap.ic_deal_placeholder)).thumbnail(0.1f).into(imageView);
    }

    public static void selectPic(ImageView imageView, String selectPic) {
        Glide.with(imageView).load(selectPic).apply((BaseRequestOptions<?>) new RequestOptions().centerCrop().placeholder(R.mipmap.ic_pic_add).error(R.mipmap.ic_pic_add)).thumbnail(0.1f).into(imageView);
    }
    ...
    public static void setRvData(RecyclerView rv, List rvData) {
        ...
        if (baseQuickAdapter != null) {
            baseQuickAdapter.setNewInstance(rvData);
        }
    }
    ...
    public static void setBanner(ConvenientBanner banner, List data) {
        ...
        }, data).setPointViewVisible(true).setPageIndicator(new int[]{R.mipmap.ic_indicator, R.mipmap.ic_indicator_true}).startTurning(2000L);
    }
    ...
    */
    ...
    public static void setDeleteLine(TextView tv2, boolean deleteLine) {
        if (deleteLine) {
    ...
    public static void setTags(LinearLayout linearLayout, List<GameBean.Tag> tags) {
        ...
        layoutGameTagBinding.setData(tags);
    ...
    public static void setTags2(LinearLayout linearLayout, List<String> tags) {
        ...
        LayoutGameTag2Binding layoutGameTag2Binding = (LayoutGameTag2Binding) DataBindingUtil.inflate(LayoutInflater.from(context), R.layout.layout_game_tag2, null, false);
        layoutGameTag2Binding.setData(tags);
        linearLayout.addView(layoutGameTag2Binding.getRoot());
    }
    ...
    public static void setTags3(LinearLayout linearLayout, List<GameBean.Tag> tags) {
        ...
        LayoutGameTag3Binding layoutGameTag3Binding = (LayoutGameTag3Binding) DataBindingUtil.inflate(LayoutInflater.from(context), R.layout.layout_game_tag3, null, false);
        layoutGameTag3Binding.setData(tags);
        linearLayout.addView(layoutGameTag3Binding.getRoot());
    }
    ...
    public static void setLogColor(TextView tv2, int logColor) {
        if (logColor == -1) {
            tv2.setTextColor(SupportMenu.CATEGORY_MASK);
            return;
        }
        if (logColor == 11) {
            tv2.setTextColor(tv2.getResources().getColor(R.color.colorPrimary));
        } else if (logColor == 200) {
            tv2.setTextColor(Color.parseColor("#58BE6B"));
        } else {
            tv2.setTextColor(tv2.getResources().getColor(R.color.color_text_1));
        }
    }
}
