...
public abstract class AbstractDatabase<T extends ACMRecord> {
    ...
    private long parseIdFromCursor(Cursor cursor) {
        ...
        return cursor.getLong(cursor.getColumnIndex(com.bytedance.framwork.core.sdklib.DBHelper.COL_ID));
    }
    ...
    protected synchronized boolean deleteOldest(SQLiteDatabase sQLiteDatabase, int i) throws DbException {
        try {
            ...
            Cursor cursorQuery = sQLiteDatabase.query(this.mTableName, new String[]{com.bytedance.framwork.core.sdklib.DBHelper.COL_ID}, null, null, null, null, "timestamp ASC", i > 0 ? String.valueOf(i) : null);
    ...
    protected int getCount(SQLiteDatabase sQLiteDatabase) {
        return (int) DatabaseUtils.longForQuery(sQLiteDatabase, String.format("SELECT COUNT(%s) FROM %s", com.bytedance.framwork.core.sdklib.DBHelper.COL_ID, this.mTableName), null);
    }
    ...
    */
    ...
    public synchronized long queryFailedMaxId() {
        long j;
        try {
            Cursor cursorQuery = getReadDatabase().query(true, this.mTableName, new String[]{com.bytedance.framwork.core.sdklib.DBHelper.COL_ID}, "upload_flag=?", new String[]{"1"}, null, null, "_id desc", null);
        ...
        if (list != null) {
            try {
                if (list.size() != 0) {
                    ...
                    String[] strArr = new String[arrayList2.size()];
                    String str = String.format("Update %s SET %s=?, %s=?, %s= %s+? where %s in %s", this.mTableName, "timestamp", DBHelpTool.RecordEntry.COLUMN_UPLOAD_FLAG, DBHelpTool.RecordEntry.COLUMN_UPLOAD_COUNT, DBHelpTool.RecordEntry.COLUMN_UPLOAD_COUNT, com.bytedance.framwork.core.sdklib.DBHelper.COL_ID, contactIds(arrayList.size()));
                    arrayList2.toArray(strArr);
