...
public class GPUImageBeautyHoliday extends GPUImageFilter {
    public static final String LookUpTableShader = "precision mediump float;\nvarying vec2 vTexCoord;\nuniform sampler2D uTexture0;\nuniform sampler2D uTextureTable;\nuniform float uIntensity;\nvoid main()\n{\n    mediump vec4 textureColor = texture2D(uTexture0, vTexCoord);\n    mediump float blueColor = textureColor.z * 63.0;\n    mediump vec2 quad1;\n    quad1.y = floor(floor(blueColor) / 8.0);\n    quad1.x = floor(blueColor) - (quad1.y * 8.0);\n    highp vec2 quad2;\n    quad2.y = floor(ceil(blueColor) / 8.0);\n    quad2.x = ceil(blueColor) - (quad2.y * 8.0);\n    highp vec2 texPos1;\n    lowp vec3 whiteColor = textureColor.xyz;\n    texPos1 = (quad1.xy * 0.125) + 0.5/512.0 + ((0.125 - 1.0/512.0) * whiteColor.xy);\n    highp vec2 texPos2;\n    texPos2 = (quad2.xy * 0.125) + 0.5/512.0 + ((0.125 - 1.0/512.0) * whiteColor.xy);\n    lowp vec4 newColor1 = texture2D(uTextureTable, texPos1);\n    lowp vec4 newColor2 = texture2D(uTextureTable, texPos2);\n    lowp vec4 newColor = mix(newColor1, newColor2, fract(blueColor));\n    mediump vec3 ycrcb;\n    ycrcb.x = pow (textureColor.x ,0.9);\n    ycrcb.y = pow (textureColor.y ,0.9);\n    ycrcb.z = pow (textureColor.z ,0.9);\n    ycrcb = (mat3(0.299, 0.587, 0.114, -0.1687, -0.3313, 0.5, 0.5, -0.4187, -0.0813 ) * ycrcb);\n    float cb = ycrcb.y * 255.0 + 128.0;\n    float cr = ycrcb.z * 255.0 + 128.0;\n    lowp float Intensity;\n    if (  ( cb  < 160.0  && cb  > 100.0 )  ) \n\t{\n\t\t  Intensity = (0.9*uIntensity * pow((1.0 - abs ( cb - 130.0 ) / 30.0) ,0.8)) ;\n    } \n\telse \n\t{\n\t\t Intensity = (uIntensity * 0.0);\n    };\n    gl_FragColor = vec4(mix(whiteColor, newColor.xyz, Intensity), 1.0); \n}";
    public static final String MixShader = "precision mediump float;\nvarying vec2 vTexCoord;\nuniform sampler2D uTexture0;\nuniform sampler2D uTexture1;\nuniform float uOpacity;\nvoid main ()\n{\n  lowp vec4 temp1;\n  lowp vec4 temp2;\n  temp1 = texture2D (uTexture0, vTexCoord);\n  temp2 = texture2D (uTexture1, vTexCoord);\n  lowp vec4 temp3;\n  temp3.w = 1.0;\n  temp3.xyz = mix(temp2.xyz, mix(temp2.xyz, min(max (((temp2.xyz + (vec3(2.0, 2.0, 2.0) * temp1.xyz)) -\n\t\tvec3(1.0, 1.0, 1.0)), 0.0), 1.0), 0.30), 0.5*uOpacity);\n  gl_FragColor = temp3;\n}";
    public static String NoChangeFrameShader = "precision mediump float;\nvarying vec2 vTexCoord;\nuniform sampler2D uTexture0;\n\nvoid main()\n{\n    vec4 color = texture2D(uTexture0, vTexCoord);\n    gl_FragColor = color;\n}";
    public static final String RgbToYuvShader = "precision mediump float;\nvarying vec2 vTexCoord;\nuniform sampler2D uTexture0;\nvoid main ()\n{\n  mediump vec4 rgb;\n  mediump vec3 yuv;\n  rgb = texture2D (uTexture0, vTexCoord);\n  yuv = (mat3(0.2126, 0, 0, 0.7152, 0, 0, 0.0722, 0, 0) * rgb.xyz);\n  gl_FragColor = vec4(yuv,1.0);\n}";
