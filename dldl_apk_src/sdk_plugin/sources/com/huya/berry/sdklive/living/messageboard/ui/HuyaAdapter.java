...
public abstract class HuyaAdapter<T> extends BaseAdapter implements Filterable {
    ...
    public HuyaAdapter(Context context, int i, List<T> list) {
        this(context, list, i);
    }

    public HuyaAdapter(Context context, int... iArr) {
        this(context, new ArrayList(), iArr);
    }

    public HuyaAdapter(Context context, List<T> list, int... iArr) {
    ...
    private class ArkAdapterFilter extends Filter {
        ...
        protected Filter.FilterResults performFiltering(CharSequence charSequence) {
            ...
            HuyaAdapter.this.mConstraint = charSequence;
            Filter.FilterResults filterResults = new Filter.FilterResults();
            if (HuyaAdapter.this.mOriginalValues == null) {
                synchronized (HuyaAdapter.this.mLock) {
                    HuyaAdapter.this.mOriginalValues = new ArrayList(HuyaAdapter.this.mObjects);
                }
            }
            if (charSequence == null || charSequence.length() == 0) {
                synchronized (HuyaAdapter.this.mLock) {
                    arrayList = new ArrayList(HuyaAdapter.this.mOriginalValues);
                }
                filterResults.values = arrayList;
                filterResults.count = arrayList.size();
            } else {
                synchronized (HuyaAdapter.this.mLock) {
                    arrayList2 = new ArrayList(HuyaAdapter.this.mOriginalValues);
                }
                int size = arrayList2.size();
                ArrayList arrayList3 = new ArrayList();
                for (int i = 0; i < size; i++) {
                    Object obj = arrayList2.get(i);
                    if (HuyaAdapter.this.conformFilter(obj, charSequence)) {
                        arrayList3.add(obj);
                    }
                }
        ...
        protected void publishResults(CharSequence charSequence, Filter.FilterResults filterResults) {
            HuyaAdapter.this.mObjects = (List) filterResults.values;
            if (filterResults.count > 0) {
                HuyaAdapter.this.notifyDataSetChanged();
            } else {
                HuyaAdapter.this.notifyDataSetInvalidated();
            }
        }
    }
}
