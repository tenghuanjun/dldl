...
import com.taptap.sdk.okhttp3.Cookie;
...
public final class BridgeInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (request.header(NSFunction.ACCEPT_ENCODING) == null && request.header("Range") == null) {
            z = true;
        ...
        if (z && NSFunction.GZIP.equalsIgnoreCase(responseProceed.header(NSFunction.CONTENT_ENCODING)) && HttpHeaders.hasBody(responseProceed)) {
            GzipSource gzipSource = new GzipSource(responseProceed.body().source());
            builderRequest.headers(responseProceed.headers().newBuilder().removeAll(NSFunction.CONTENT_ENCODING).removeAll("Content-Length").build());
