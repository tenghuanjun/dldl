...
class GroupingKt__GroupingKt extends GroupingKt__GroupingJVMKt {
    ...
    public static final <T, K, R> Map<K, R> fold(Grouping<T, ? extends K> grouping, R r, Function2<? super R, ? super T, ? extends R> operation) {
        ...
        while (itSourceIterator.hasNext()) {
            ...
            Manifest.permission permissionVar = (Object) linkedHashMap.get(kKeyOf);
            if (permissionVar == null && !linkedHashMap.containsKey(kKeyOf)) {
                permissionVar = (Object) r;
            }
            linkedHashMap.put(kKeyOf, operation.invoke(permissionVar, next));
        }
        return linkedHashMap;
    }
    ...
    public static final <T, K, R, M extends Map<? super K, R>> M foldTo(Grouping<T, ? extends K> grouping, M destination, R r, Function2<? super R, ? super T, ? extends R> operation) {
        ...
        while (itSourceIterator.hasNext()) {
            ?? next = itSourceIterator.next();
            K kKeyOf = grouping.keyOf(next);
            Manifest.permission permissionVar = (Object) destination.get(kKeyOf);
            if (permissionVar == null && !destination.containsKey(kKeyOf)) {
                permissionVar = (Object) r;
            }
            destination.put(kKeyOf, operation.invoke(permissionVar, next));
        }
        return destination;
    }
