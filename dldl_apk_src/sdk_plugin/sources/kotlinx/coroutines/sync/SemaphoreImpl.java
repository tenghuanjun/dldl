        ...
        do {
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    objM1597constructorimpl = SegmentOrClosed.m1597constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (SegmentOrClosed.m1602isClosedimpl(objM1597constructorimpl)) {
                break;
            }
            Segment segmentM1600getSegmentimpl = SegmentOrClosed.m1600getSegmentimpl(objM1597constructorimpl);
            while (true) {
                Segment segment = (Segment) this.tail;
                if (segment.getId() >= segmentM1600getSegmentimpl.getId()) {
                    break;
                }
                if (!segmentM1600getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    break;
                }
                if (tail$FU.compareAndSet(this, segment, segmentM1600getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM1600getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    segmentM1600getSegmentimpl.remove();
                }
            }
            z = true;
        } while (!z);
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m1600getSegmentimpl(objM1597constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        do {
            ...
            while (true) {
                ...
                if (head$FU.compareAndSet(this, segment, segmentM1600getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM1600getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
