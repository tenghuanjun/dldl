...
public class ThreadUtils {
    ...
    private static final Executor EXECUTOR = Executors.newFixedThreadPool(4, new ThreadFactory() { // from class: notchtools.geek.com.notchtools.helper.ThreadUtils.2
        private int count = -1;
    ...
    public static void delayCancelLogicTask(Context context, long j) {
        if (context != null) {
            final Integer numValueOf = Integer.valueOf(context.hashCode());
            postDelayed2Logic(new Runnable() { // from class: notchtools.geek.com.notchtools.helper.ThreadUtils.1
                @Override // java.lang.Runnable
                public void run() {
                    ThreadUtils.getLogicThreadHandler().removeCallbacksAndMessages(numValueOf);
                }
            }, j);
        }
    }
    ...
    public static <T> void exec(final Provider<T> provider, final Consumer<T> consumer) {
        ...
        EXECUTOR.execute(new Runnable() { // from class: notchtools.geek.com.notchtools.helper.ThreadUtils.3
            @Override // java.lang.Runnable
            public void run() {
                final Object objProvide;
                try {
                    objProvide = provider.provide();
                } catch (Throwable th) {
                    th.printStackTrace();
                    objProvide = null;
                }
                ...
                ThreadUtils.post2UI(new Runnable() { // from class: notchtools.geek.com.notchtools.helper.ThreadUtils.3.1
                    @Override // java.lang.Runnable
                    public void run() {
                        try {
                            consumer.consume(objProvide);
                        } catch (Throwable th2) {
                            th2.printStackTrace();
                        }
                    }
