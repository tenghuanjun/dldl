...
final class ContinuationMigration<T> implements Continuation<T> {
    ...
    public void resumeWith(@NotNull Object result) {
        if (Result.m25isSuccessimpl(result)) {
            this.continuation.resume(result);
        }
        Throwable thM21exceptionOrNullimpl = Result.m21exceptionOrNullimpl(result);
        if (thM21exceptionOrNullimpl != null) {
            this.continuation.resumeWithException(thM21exceptionOrNullimpl);
        }
    }
}
