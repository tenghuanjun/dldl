...
final class Blend {
    ...
    public static int harmonize(int i, int i2) {
        ...
        return Hct.from(MathUtils.sanitizeDegrees(hctFromInt.getHue() + (Math.min(MathUtils.differenceDegrees(hctFromInt.getHue(), hctFromInt2.getHue()) * HARMONIZE_PERCENTAGE, HARMONIZE_MAX_DEGREES) * rotationDirection(hctFromInt.getHue(), hctFromInt2.getHue()))), hctFromInt.getChroma(), hctFromInt.getTone()).toInt();
    }
