...
public abstract class d {
    ...
    private static byte[] a(String str, byte[] bArr, byte[] bArr2) {
        if (!TextUtils.isEmpty(str) && c(bArr) && b(bArr2) && d()) {
            try {
                return a(str.getBytes(Constants.ENC_UTF_8), bArr, bArr2);
    ...
    public static byte[] b(String str) throws UnsupportedEncodingException {
        ...
        byte[] bytes = upperCase.getBytes(Constants.ENC_UTF_8);
        for (int i = 0; i < length; i++) {
            int i2 = i * 2;
            bArr[i] = (byte) (((byte) (Byte.decode("0x".concat(new String(new byte[]{bytes[i2]}, Constants.ENC_UTF_8))).byteValue() << 4)) ^ Byte.decode("0x".concat(new String(new byte[]{bytes[i2 + 1]}, Constants.ENC_UTF_8))).byteValue());
        }
        return bArr;
    }
    ...
    private static SecureRandom c() {
        SecureRandom secureRandomM;
        try {
            secureRandomM = Build.VERSION.SDK_INT >= 26 ? x$.ExternalSyntheticApiModelOutline0.m() : SecureRandom.getInstance("SHA1PRNG");
        } catch (Exception e) {
            Log.w("Aes128", "getInstanceStrong, exception: " + e.getClass().getSimpleName());
            secureRandomM = null;
        }
        return secureRandomM == null ? new SecureRandom() : secureRandomM;
    }
    ...
    private static String d(String str, byte[] bArr) {
        if (!TextUtils.isEmpty(str) && bArr != null && bArr.length >= 16 && d()) {
            try {
                ...
                return new String(cipher.doFinal(a(strD)), Constants.ENC_UTF_8);
