...
public class ScrollUtils {
    ...
    static int computeVerticalScrollOffset(View view) {
        ScrollingView scrolledView = getScrolledView(view);
        if (scrolledView instanceof ScrollingView) {
            return scrolledView.computeVerticalScrollOffset();
        }
        try {
    ...
    static int computeVerticalScrollRange(View view) {
        ScrollingView scrolledView = getScrolledView(view);
        if (scrolledView instanceof ScrollingView) {
            return scrolledView.computeVerticalScrollRange();
        }
        try {
    ...
    static int computeVerticalScrollExtent(View view) {
        ScrollingView scrolledView = getScrolledView(view);
        if (scrolledView instanceof ScrollingView) {
            return scrolledView.computeVerticalScrollExtent();
        }
        try {
    ...
    static boolean canScrollVertically(View view, int direction) {
        ...
        int itemCount;
        int i;
        RecyclerView scrolledView = getScrolledView(view);
        ...
        if (scrolledView instanceof RecyclerView) {
            RecyclerView recyclerView = scrolledView;
    ...
    static int getRawX(View rootView, MotionEvent ev, int pointerIndex) {
        ...
        if (Build.VERSION.SDK_INT >= 29) {
            x = RenderNodeApi29$.ExternalSyntheticApiModelOutline0.m(ev, pointerIndex);
        } else {
    ...
    static int getRawY(View rootView, MotionEvent ev, int pointerIndex) {
        float y;
        if (Build.VERSION.SDK_INT >= 29) {
            y = RenderNodeApi29$.ExternalSyntheticApiModelOutline0.m$1(ev, pointerIndex);
        } else {
            rootView.getLocationOnScreen(new int[2]);
