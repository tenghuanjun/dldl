    ...
    public static <T, U> void drainMaxLoop(SimplePlainQueue<T> q, Subscriber<? super U> a, boolean delayError, Disposable dispose, QueueDrain<T, U> qd) {
        ...
        while (true) {
            ...
            if (checkTerminated(zDone, z, a, delayError, q, qd)) {
                ...
                if (jRequested != 0) {
                    if (qd.accept(a, tPoll) && jRequested != REQUESTED_MASK) {
                        qd.produced(1L);
                    }
                } else {
                    q.clear();
    ...
    */
    ...
    public static <T> boolean postCompleteRequest(long n, Subscriber<? super T> actual, Queue<T> queue, AtomicLong state, BooleanSupplier isCancelled) {
        ...
        do {
            j = state.get();
            j2 = REQUESTED_MASK & j;
        } while (!state.compareAndSet(j, BackpressureHelper.addCap(j2, n) | (j & COMPLETED_MASK)));
        if (j != COMPLETED_MASK) {
            return false;
        }
    ...
    static <T> boolean postCompleteDrain(long n, Subscriber<? super T> actual, Queue<T> queue, AtomicLong state, BooleanSupplier isCancelled) {
        ...
        while (true) {
            if (j != n) {
                ...
                n = state.get();
                if (n == j) {
                    long jAddAndGet = state.addAndGet(-(j & REQUESTED_MASK));
                    if ((REQUESTED_MASK & jAddAndGet) == 0) {
                        return false;
                    }
                    n = jAddAndGet;
                    j = jAddAndGet & COMPLETED_MASK;
                } else {
                    continue;
                }
            }
